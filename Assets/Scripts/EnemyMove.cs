...
public class EnemyMove : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.name == "Pacman")
        {
            if (GameManager.Instance.isSuperPacman)
            {
                ...
                index = 0;
                this.enabled = false;//关闭该脚本的运行
                GetComponent<Animator>().SetBool("Die", true);//设置为死亡状态
                //无敌期间吃到鬼加的得分
                GameManager.Instance.score += 50;
                Invoke("DelayedResetEnemy", 2f);
            }
            else
            {
                ...
                //玩家掉血
                GameManager.Instance.UpdatePlayerHealth(-1);//掉1格血
                /*
                //游戏结束，先隐藏积分面板
    ...
    public void DelayedResetEnemy()
    {
        this.enabled = true;//开启该脚本的运行
        GetComponent<Animator>().SetBool("Die", false);//设置为死亡状态
    }
